...
/** One chord held for a whole number of bars. Roots are stored international, shown German. */
...
export function buildProgression(progression: Progression, key: string): HarmonyStep[] {
...
export function harmonyAt(steps: readonly HarmonyStep[], bar: number) {
  const total = steps.reduce((sum, item) => sum + item.bars, 0);
  if (!total) return undefined;
  let position = mod(bar, total);
  for (let index = 0; index < steps.length; index++) {
    if (position < steps[index].bars)
      return { index, step: steps[index], first: position === 0, localBar: position };
    position -= steps[index].bars;
  }
  return undefined;
}

export const harmonyBars = (steps: readonly HarmonyStep[]) =>
  steps.reduce((sum, item) => sum + item.bars, 0);
...
export function normalizeHarmony(value: unknown): Harmony {
  ...
  const steps = Array.isArray(data.steps)
    ? data.steps.slice(0, 32).flatMap((item) => {
        ...
        return pitchClass(root) < 0
          ? []
          : [{ root, chordId: chord.id, bars: Math.round(clamp(entry.bars, 1, 8, 1)) }];
      })
    : [];
  return {
